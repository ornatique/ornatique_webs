                            ...
                            <tbody>
                                @foreach ($data as $key)
                                    <tr id="tr{{ $key->id }}">
                                        ...
                                        <td><img width='80'
                                                src="{{ asset('public/assets/images/categories') . '/' . $key->image }}"
                                                alt="">
                                        </td>
                                        <td>{{ ucwords($key->name) }}</td>
                                        <td>
                                            <span
                                                style="background-color: {{ $key->color }}; color: white; padding: 4px 8px; border-radius: 4px;">
                                                {{ $key->color ? $key->color : 'N/A' }}
                                            </span>
                                        </td>
                                        <td>{{ $key->priority }}</td>
                                        <td>{{ $key->shape ? $key->shape : 'N/A' }}</td>
                                        <td> <a href="{{ url('admin/category/edit') . '/' . $key->id }}"
                    ...
                    <div class="modal fade" id="DeletePopup" tabindex="-1" role="dialog"
                        ...
                        <div class="modal-dialog modal-dialog-centered" role="document">
                            <div class="modal-content">
                                ...
                                <form class="user" action="{{ url('admin/category/delete') }}" method="POST">
                                    @csrf
