...
class OrederController extends Controller
{
        ...
        if ($validatiion->fails()) {
            ...
            if (count($data) > 0) {
                foreach ($data as $key) {
                    ...
                    if ($key->product) {
                        $order->weight = $key->product->weight;
                    }
    ...
    public function order_list(Request $request)
    {
        ...
        $data = Order::where('user_id', $request->user_id)->with('product')->groupBy('order_id')->orderBy('updated_at', 'DESC')
            // ->select('*', DB::raw('CAST(SUM(weight) AS DECIMAL(10,3))as total'))
            ->select('*', DB::raw('CAST(weight * quantity AS DECIMAL(10,3))as total'))
